"""Self-tests for the REMOTE TEMP reaper — one session-end sweep (SPEC §11.5) — offline, fake reaper.

The driver reaps REMOTE TEMP once at ``sessionfinish`` (controller-only): it sweeps the session prefix
``<root>/<session-id>/``, sparing the batches of FAILED tests (the keep-list). Remoteness comes from
``--temp-dir-base`` (the driver's ``<root>``), NOT a driver-set env var, and reaping is scoped to TEMP
only — DATA is never reaped. These exercise the framework's *when* + keep-list against a FAKE reaper
...
rmdirs``) — the reaper's *how*. And the per-test-leaf keep-list refinement (keep only a failed
``<batch>/<test-id>/`` instead of the whole ``<batch>/``) needs the binary's ``TEST_ID`` (or a
``.failed-dirs`` file it writes) to stay under-reap-safe — a follow-up, not the framework's job here.
"""
...
from ducktest.reaper import age_sweep, record_failure, sweep_session
...
class FakeSweepReaper(FakeReaper):
    """A reaper that also lists session prefixes (drives the age-sweep). A prefix ending in ``explode``
...
class Cfg:
    ...
    the options the reaper reads. ``temp_dir_base`` is the ``<root>`` (remote → the reaper fires; local
    → it no-ops)."""
...
def _config(reaper=None, node_batch_ids=None, **kw):
    cfg = Cfg(**kw)
...
# -----------------------------------------------------------------------------
# The one session-end sweep — keep-on-failure keep-list, remote-only, reaper-gated
#


def test_all_pass_sweeps_with_empty_keeplist(clean_env):
    # No failures recorded → keep nothing → everything under the session prefix is reaped.
    reaper = FakeReaper()
    cfg = _config(reaper, node_batch_ids={"t/a.test": "batch-0"})
    sweep_session(cfg)
    assert reaper.swept == [(_session_prefix(), [])]
...
def test_failed_test_keeps_its_batch(clean_env):
    ...
    sweep_session(cfg)
    assert reaper.swept == [(_session_prefix(), ["batch-2/**"])]


def test_multiple_failed_batches_each_kept(clean_env):
    reaper = FakeReaper()
    cfg = _config(reaper, node_batch_ids={"t/a.test": "batch-2", "t/c.test": "batch-5"})
    record_failure(cfg, Rep("t/a.test", failed=True))
    record_failure(cfg, Rep("t/c.test", failed=True))
    sweep_session(cfg)
    assert reaper.swept == [(_session_prefix(), ["batch-2/**", "batch-5/**"])]


def test_record_failure_ignores_passes(clean_env):
    reaper = FakeReaper()
    cfg = _config(reaper, node_batch_ids={"t/a.test": "batch-1"})
    record_failure(cfg, Rep("t/a.test", failed=False))  # a pass is not recorded
    sweep_session(cfg)
    assert reaper.swept == [(_session_prefix(), [])]  # nothing kept → all reaped
...
def test_unmapped_failure_keeps_whole_session(clean_env):
    ...
    reaper = FakeReaper()
    cfg = _config(reaper, node_batch_ids={"t/a.test": "batch-2"})
...
def test_no_batch_map_keeps_whole_session_on_failure(clean_env):
    # No plan/map at all + a failure → keep everything (never over-reap a failed test's artifacts).
    reaper = FakeReaper()
    cfg = _config(reaper, node_batch_ids=None)
    record_failure(cfg, Rep("t/a.test", failed=True))
    sweep_session(cfg)
    assert reaper.swept == [(_session_prefix(), ["**"])]
...
def test_local_root_is_noop(clean_env):
    ...
    reaper = FakeReaper()
    cfg = _config(reaper, node_batch_ids={"t/a.test": "batch-0"}, temp_dir_base="/tmp/base")
    record_failure(cfg, Rep("t/a.test", failed=True))
    sweep_session(cfg)
    assert reaper.swept == []


def test_no_reaper_is_noop(clean_env):
    cfg = _config(reaper=None, node_batch_ids={"t/a.test": "batch-0"})  # remote, but nothing registered
    record_failure(cfg, Rep("t/a.test", failed=True))
    sweep_session(cfg)  # must not raise


def test_interruption_without_sessionfinish_does_no_sweep(clean_env):
    # A run that never reaches sessionfinish never calls sweep_session → the reaper is untouched (the
...
def test_age_sweep_purges_only_stale_prefixes(clean_env):
    ...
    assert reaper.swept == [(f"{REMOTE}/{old}", [])]
...
def test_age_sweep_tolerates_bad_prefix_and_sweep_error(clean_env):
    ...
    assert reaper.swept == [(f"{REMOTE}/{old}", [])]  # the good stale one still got swept


def test_age_sweep_noop_without_list_support(clean_env):
    # A reaper without list_run_prefixes → the age-sweep is silently skipped (the session sweep still works).
    reaper = FakeReaper()
    cfg = _config(reaper)
    age_sweep(cfg)
    assert reaper.swept == []
...
def test_age_sweep_local_root_is_noop(clean_env):
    reaper = FakeSweepReaper([(f"{REMOTE}/{_session_id(99)}", _session_id(99))])
...
def test_session_sweep_wired_through_real_pytest(pytester, monkeypatch):
    ...
    conftest registers a reaper that logs its sweep call; one test passes, one fails. Remoteness comes
    from a remote --temp-dir-base. The failing tests are plain `.py` (not batched SqlLogic items), so
    they aren't in the node→batch map → the sweep keeps the WHOLE session (the coarse under-reap-safe
    ...
    assert len(lines) == 1  # exactly one session-end sweep (the age-sweep no-ops: LogReaper can't list)
    prefix, keeps = json.loads(lines[0])
    assert prefix.startswith(REMOTE + "/") and prefix.endswith("/")  # <root>/<session-id>/
    assert keeps == ["**"]  # a failure with no batch map → keep the whole session (under-reap safe)
